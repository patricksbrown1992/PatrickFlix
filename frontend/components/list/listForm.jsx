...
const ListForm = props => {
    ...
    lists = lists.map(list => {
        return <div id ={`${list.id}-list-div`} onMouseOut={hideButton} onMouseOver={showButton} key = {list.id} className='list-div'> 
            <Link   id='list-li' to={`/player/${videos[list.video_id].id}`}><img src={videos[list.video_id].image_link} /></Link>
            <h3 className='list-video-h3'>{videos[list.video_id].title}</h3><div  id='button-background'><i onClick={removeList} id={`${list.video_id}-${list.id}-list-check`} className="fas fa-check hidden"></i></div><div id='remove-from-list'  >Remove from My List</div>
        </div>
                ...
                <div className='right-nav'>
                    ...
                    <div id='search-form'>
                        <div id= 'search-div'>
                            ...
                            <input className={searchClass} input = 'search-input' type="text" name="search" value={search} onChange = {changeUpdate} placeholder="Titles" />
